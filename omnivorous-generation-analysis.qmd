...
# Add new assumption: Linear negative cohort trend between 1890 and 1990
# This implies c*(1990) <= c*(1890), yielding another upper bound on s.
s_upper_cohort_o <- as.numeric((cohort_coefs_omni["1890"] - cohort_coefs_omni["1990"]) / 100)

# Determine valid bounds
s_max_o <- min(s_upper_age_o, s_upper_period_o, s_upper_cohort_o)
...
By incorporating the additional assumption of a **linear negative cohort trend** (i.e., that the overall generational effect between the earliest cohort in 1890 and the latest in 1990 has declined), the bounds on the unknown slope parameter $s$ become much tighter and exclusively negative. Because APC models cannot identify the true linear slope due to collinearity, the bounds rotate the overall baseline trend up and down. 
As seen in the updated plot, under these tighter qualitative constraints, all mathematically possible cohort curves now show a distinct downward trajectory. Yet crucially, the non-linear structure remains perfectly stable and perfectly identified. Regardless of which slope is chosen within the bounds, the cohort effects for those born between 1915 and 1955 exhibit a massive positive deviation (a structural "bulge") relative to the baseline linear trend. The mid-century cohorts (1915-1955) clearly arrest and reverse the overall generational decline before it resumes for post-1970 generations, proving a distinctly elevated generational imprint.
